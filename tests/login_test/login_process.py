...
def loginViaQR():
    ...
    while True:
        ...
        print("check login: req.text:", req.text)
        ...
        if req.text.find("登录成功") != -1:
            ...
            print("response_header_dict:", response_header_dict)
            # 获取skey和p_skey
            print("set-cookie:", response_header_dict['Set-Cookie'])
            res = requests.get(check_sig_url.format(uin, ptsigx),
                                headers={'Cookie': response_header_dict['Set-Cookie']})

            final_cookie = res.headers['Set-Cookie']
            print("final_cookie:", final_cookie)
